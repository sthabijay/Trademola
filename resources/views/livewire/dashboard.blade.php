        ...
        <main class="bg-white w-full flex border-2 rounded-xl border-gray-300">
            <div class="w-[330px] bg-gray-50 rounded-l-2xl p-4 border-r-2 border-gray-300 shrink-0 flex flex-col">
                <div class="flex flex-col">
                    <h1 class="text-xl text-gray-700">Portfolio: {{$selected_portfolio->name}}</h1>
                    ...
                    <div class="text-3xl flex justify-between items-center">
                        @if($selected_portfolio->gross > 0)
                            ...
                            <h1 class="text-green-500 bg-green-100 flex items-center w-fit p-1 rounded-md">
                                ...
                                {{ $selected_portfolio->current_investment > 0 
                                ? number_format((($selected_portfolio->current_value - $selected_portfolio->current_investment) / $selected_portfolio->current_investment) * 100, 0) 
                                : '0.00' }}%
                            </h1>
                        @elseif($selected_portfolio->gross < 0)
                            ...
                            <h1 class="text-red-500 bg-red-100 flex items-center w-fit p-1 rounded-md">
                                ...
                                {{ $selected_portfolio->current_investment > 0 
                                ? number_format((($selected_portfolio->current_value - $selected_portfolio->current_investment) / $selected_portfolio->current_investment) * 100, 2) 
                                : '0.00' }}%
                            </h1>
                        @else
                            {{-- Neutral --}}
                ...
                <div class="pt-4 flex flex-col gap-2 overflow-scroll no-scrollbar">
                    @foreach($logs as $log)
                        <div wire:click="selectLog({{$log->id}})" class="border border-gray-500 rounded-lg p-2 hover:bg-blue-100 cursor-pointer transition-all {{$selected_log && $selected_log->id === $log->id ? 'bg-blue-50' : 'bg-white'}}">
                            <div class="flex justify-between items-center text-lg">
                                ...
                                @else
                                <span class="bg-red-200 text-red-500 rounded-lg px-2 py-1">Close</span>                                    
                                @endif
                            </div>
            ...
            @if($selected_log)
                ...
                <div>
                    <div class="text-2xl pb-2 gap-2">
                        <div class="flex gap-2">
                            {{$selected_log? \Carbon\Carbon::parse($selected_log->created_at)->format('D, M d') : ""}}
                        </div>                        
                        <div class="text-sm flex gap-2">
                            <div class="text-gray-400">Last Updated: </div>
                            {{$selected_log? \Carbon\Carbon::parse($selected_log->updated_at)->format('D, M d') : ""}}
                        </div>                        
                    </div>
                    <div class="flex items-center gap-2">
                        <h1 class="text-gray-500">Tags:</h1>
                        <span wire:click="set('showAddTag', true)" class="bg-cyan-200 text-cyan-700 rounded-lg py-1 px-2 hover:bg-cyan-300 transition-all cursor-pointer">+Add Tags</span>
                        @if($showAddTag)
                            <input wire:model="new_tag" wire:keydown.enter="addTag"  class="bg-cyan-100 text-cyan-700 rounded-lg py-1 px-2 w-auto" type="text" autofocus/>
                        @endif
                ...
                <div class="flex">
                    <div class="pr-4 w-full">
                        <div class="flex text-gray-500 gap-1 items-center justify-between">
                        <div class="flex gap-1">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-scroll-text-icon lucide-scroll-text"><path d="M15 12h-5"/><path d="M15 8h-5"/><path d="M19 17V5a2 2 0 0 0-2-2H4"/><path d="M8 21h12a2 2 0 0 0 2-2v-1a1 1 0 0 0-1-1H11a1 1 0 0 0-1 1v1a2 2 0 1 1-4 0V5a2 2 0 1 0-4 0v2a1 1 0 0 0 1 1h3"/></svg>
                            Notes:
                        </div>                        
                        <button  class="ml-4 p-1 bg-cyan-200 text-cyan-700 rounded-md hover:bg-cyan-100 cursor-pointer transition-all">
                        ...
                        @if($showNoteTextbox)
                            <textarea wire:model="new_note" class="overflow-scroll h-[550px] flex no-scrollbar">

                            </textarea>
                        @else
                            <p class="overflow-scroll h-[550px] flex no-scrollbar">
                                {{$selected_log->note}}
                            </p>
                        @endif                        
                    </div>                   
                    <div class="px-4 w-[330px] shrink-0 border-l-2 border-gray-300">
                        ...
                        <div class="grid grid-cols-2">
                            <h1 class="text-gray-500">Sell Price</h1>
                            <h1>Rs {{$selected_log->avg_sell_price}}</h1>
                        </div>
                        <div class="grid grid-cols-2">
                        ...
                        <button wire:click="$set('showEntryModel', true)" class="w-full bg-cyan-700 py-2 rounded-lg border border-cyan-700 text-white hover:bg-white hover:border-cyan-700 hover:text-cyan-700 transition-all cursor-pointer">+ Add Entries</button>
                        <div class="py-2 flex flex-col gap-2 overflow-scroll h-[300px] no-scrollbar">
                            @foreach($entries as $entry)
                                <div class="border rounded-lg p-2 {{$entry->is_buy? "border-green-500 bg-green-100" : "border-red-500 bg-red-100"}}">
                                    <div class="flex justify-between items-center">                                    
                                        <h1 class="text-gray-500">{{\Carbon\Carbon::parse($entry->updated_at)->format('D, M d')}}</h1>
                                        <span class=" rounded-lg px-2 py-1 {{$entry->is_buy? "bg-green-200 text-green-500" : "bg-red-200 text-red-500"}}">{{$entry->is_buy? "Buy" : "Sell"}}</span>
                                    </div>
                                    <div class="flex gap-1">
                                        <h1 class="text-gray-500">Price: </h1>{{$entry->price}}
                                    </div>
                                    <div class="flex gap-1">
                                        <h1 class="text-gray-500">Units: </h1>{{$entry->units}}
                                    </div>
                                </div>
                            @endforeach                            
                        </div>
